...
class DemoApp(MDApp):
    ...
    def on_start(self):
        ...
        cur = conn.cursor()
        cur.execute(""" 
                         SELECT fname, age, height, weight, heartrate, RESULT, DATE_TIME
                         FROM information INNER JOIN results
                         ON information.id = results.UID
                         """)
        output = cur.fetchall()
        self.row_data = output
        # for data in output:
        #     self.row_data = data
        self.column_data = [("Name", dp(35)), ("Age", dp(25)), ("Height", dp(25)), ("Weight", dp(25)),
                            ("Heartrate", dp(25)), ("Result", dp(25)), ("Date/Time", dp(35))]
        self.data_tables = MDDataTable(size_hint=(0.9, 0.6),use_pagination=True,
                                       column_data=self.column_data, row_data=self.row_data)
        self.root.ids.scr_mngr.get_screen('history').ids.dt_layout.add_widget(self.data_tables)
   ...
   #         (data)
    ...
    def bmidialog(self):
        close_button = MDFlatButton(text="Okay", on_release=self.close_dialog)
        self.dialog = MDDialog(title="Result Check",
                               text="If your BMI is less than 18.5, it falls within the underweight range. \n If your "
    ...
    def show_data(self, *args):
        #btn.disabled=True
        #Clock.schedule_once(partial(self.btn_enable,btn), 10)
    ...
    def okay(self, *args):
        var=(self.root.ids.scr_mngr.get_screen('input').ids.fname.text),
        var=(self.root.ids.scr_mngr.get_screen('input').ids.height.text),
        var=(self.root.ids.scr_mngr.get_screen('input').ids.weight.text),
        var= (self.root.ids.scr_mngr.get_screen('input').ids.age.text),
        var=(self.root.ids.scr_mngr.get_screen('input').ids.heartrate.text)
        self.save_data()
